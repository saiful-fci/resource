<script type="text/javascript">
    ...
    $('#section_select').on('change', function (e) {
        $.ajax({
            type: "GET",
            dataType:'json',
            url:"{{URL::to('getPayhead')}}",
            success: function(data) {
                $("#pay_head").empty();
                $("#pay_head").append('<option value="" selected disabled>Please select</option>');
                $.each(data, function (index, value) {
                    if(value.pay_head=="Transport Fee"){
                        $("#pay_head").append('<option value="' + value.id + '">' + value.pay_head + '</option>');
                    }
                });               
            },
            error: function(data) {
    ...
    $('#pay_head').on('change', function (e) {
        $.ajax({
            ...
            success: function(data) {
                $("#select_month").empty();
                $("#select_month").append('<option value="" selected disabled>Please select</option>');
                $.each(data, function (index, value) {
                    $("#select_month").append('<option value="' + value.id + '">' + value.name + '</option>');
                });               
            },
            error: function(data) {
                console.log(data);
    ...
    $('#select_month').on('change', function (e) {
        ...
        var sectionOption = $('#section_select');
        $.ajax({
            url: "{{URL::to('getStudentsForDifferPayable')}}",
            type: 'GET',
            dataType: 'json',
            ...
            success: function (data) {
                ...
                $.each(data,function (key,val) {
                    content += '<tr>'+
                            ...
                            '<input type="hidden" class="form-control" name="sarId[]" value="'+val.academic_record_id+'"/>'
